...
using Zirve.NotificationEngine.Core.Domain.Models.SmartMessage;

namespace Zirve.NotificationEngine.Core.NotificationPublisher.Implementations.SmsPublisher.Implementations
{
    public class SmartMessageSmsNotificationPublisher : ISmsNotificationPublisher
    {
        private static ILogger _logger;


        public SmartMessageSmsNotificationPublisher(ILogger logger)
        {
            _logger = logger;
        }

        #region ISmsNotificationPublisher Members
        ...
        public PublishResponse Publish(PublishRequest request)
        {
            ...
            #region Build Sms Request XML

            var xmlBuilder = new StringBuilder();

            xmlBuilder.Append("<SENDSMS>");
            ...
            xmlBuilder.AppendFormat("<MSG>{0}</MSG>", request.Message);
            xmlBuilder.AppendFormat("<LNG>{0}</LNG>", "TR");
            ...
            if (!isBulk)
            {
                var numbers = request.MessageTargetIdentifier.Replace(";", ",").Split(',');
                for (int i = 0; i < numbers.Length; i++)
                {
                    xmlBuilder.Append("<RCPT>");
                    xmlBuilder.AppendFormat("<TA>{0}</TA>", numbers[i]);
            ...
            try
            {
                response = SendSmartMessageSMS(xmlBuilder.ToString());
            }
            catch (Exception e)
            {
            ...
            {
                int count;
                var success = Int32.TryParse(response.SUCCESSCOUNT, out count);
                if (success && count > 0)
                {
                    return new PublishResponse()
                    {
                        MessageId = response.RSP_LIST[0].MSGID,
                        ResponseCode = ResponseCode.Success,
                        Explanation = response.EXP,
                        SmartMessageReturnCode = response.RTCD
                    };
                }
                ...
                if (response.RTCD != "1")
                {
                    return new PublishResponse()
                    {
                        MessageId = response.RSP_LIST.Count > 0 ? response.RSP_LIST[0].MSGID : string.Empty,
                        ResponseCode = ResponseCode.SmartMessageSendEmailServiceError,
                        Explanation = response.EXP,
                        SmartMessageReturnCode = response.RTCD
                    };
                }
        ...
        public InquiryResponse Inquiry(InquiryRequest request)
        {
            ...
            xmlBuilder.Append("</EIDLIST>");
            xmlBuilder.Append("<MSGLIST>");
            xmlBuilder.AppendFormat("<MSGID>{0}</MSGID>", request.MessageId);
            xmlBuilder.Append("</MSGLIST>");
        ...
        public Client.Enumerations.SmsPublishType SmsNotificationPublishType
        {
            get { return Client.Enumerations.SmsPublishType.SmartMessage; }
        }
        ...
        static SENDSMSRSP SendSmartMessageSMS(string requestXml)
        {
            var response = new SENDSMSRSP();
            using (var client = GetSmartMessageSoapClient())
            {
                _logger.Log(String.Format("Smart Message SendSmartMessageSMS Request: {0}", requestXml), LogType.Information);
                _logger.Log("-------------------------------", LogType.Information);

                var serviceGetTokenResponse = client.SENDSMS(requestXml);
                _logger.Log(String.Format("Smart Message SendSmartMessageSMS Response: {0}", serviceGetTokenResponse), LogType.Information);
                response = Deserialize<SENDSMSRSP>(serviceGetTokenResponse);
        ...
        static int Inquiry(string requestXml, out string statusDetail)
        {
            ...
            using (var client = GetSmartMessageSoapClient())
            {
                ...
                {
                    var responseModel = Deserialize<GETREPORTRSP>(serviceGetTokenResponse);
                    DateTime maxDateTime = default(DateTime);
                    string laststatu = String.Empty;

                    if (responseModel.RSP_LIST.RSP.Count > 0)
                    {
                        foreach (var item in responseModel.RSP_LIST.RSP)
                        {
                            if (!string.IsNullOrWhiteSpace(item.SDATE))
                            {


                                DateTime dateTimeResult = default(DateTime);
                                var ci = new CultureInfo("de-DE");
                                _logger.Log(String.Format("DateTimeConvert Result: {0}", DateTime.TryParseExact(item.SDATE, "g", ci, DateTimeStyles.None, out dateTimeResult).ToString()), LogType.Information);

                                if (DateTime.TryParseExact(item.SDATE, "g", ci, DateTimeStyles.None, out dateTimeResult))
                                {
                                    if (dateTimeResult > maxDateTime)
                                    {
                                        maxDateTime = dateTimeResult;
                                        laststatu = item.ST;
                    ...
                    _logger.Log(String.Format("Smart Message GETREPORT Last status: {0}", laststatu), LogType.Information);

                    string status = !string.IsNullOrWhiteSpace(laststatu) ? laststatu : string.Empty;

                    _logger.Log(String.Format("Smart Message GETREPORT STATUS: {0}", status), LogType.Information);
                    ...
                    else if (status == "SNT" || status == "ANK" || status == "JNK")
                    {
                        statusCode = NotificationStatusCode.Completed;
                             ...
                             status == "BLK" || status == "ABN" || status == "EBN" || status == "DUP" || status == "SXP" || status == "FLX")
                    {
                    ...
                    switch (status)
                    {
                        ...
                        case "UKN":
                            ...
                            break;
                        case "OPN":
                            statusDetail = "Başarıyla iletilmiş mesaj";
                            break;
                        case "SNT":
                        ...
                        case "DUP":
                            ...
                            break;
                        case "FLX":
                            statusDetail = "Operatörden hata aldığına dair rapor alınmış mesaj";
                            break;
                        case "SXP":
                            statusDetail = "Zaman aşımına uğramış mesaj";
                            break;
                        default:
            ...
            _logger.Log(String.Format("Smart Message GETREPORT STATUSCODE: {0}", statusCode), LogType.Information);
